...
class BinTree : InWindow() {
    ...
    var _matchMaxLen = 0
    lateinit var _son: IntArray
    lateinit var _hash: IntArray
    var _cutValue = 0xFF
    var _hashMask = 0
    var _hashSizeSum = 0
    var HASH_ARRAY = true
    ...
    fun Create(
        ...
        if (_cyclicBufferSize != cyclicBufferSize) _son = IntArray(cyclicBufferSize.also {
            _cyclicBufferSize = it
        } * 2)
        var hs = kBT2HashSize
        if (HASH_ARRAY) {
            hs = historySize - 1
    ...
    fun GetMatches(distances: IntArray): Int {
        ...
        if (HASH_ARRAY) {
            ...
            temp = temp xor ((_bufferBase!![cur + 2] and 0xFF) as Int shl 8)
            hash3Value = temp and kHash3Size - 1
            hashValue = temp xor (CrcTable[_bufferBase!![cur + 3] and 0xFF] shl 5) and _hashMask
        } else hashValue = _bufferBase!![cur] and 0xFF xor ((_bufferBase!![cur + 1] and 0xFF) as Int shl 8)
        ...
        while (true) {
            ...
            val cyclicPos =
                (if (delta <= _cyclicBufferPos) _cyclicBufferPos - delta else _cyclicBufferPos - delta + _cyclicBufferSize) shl 1
            val pby1: Int = _bufferOffset + curMatch
            var len: Int = min(len0, len1)
