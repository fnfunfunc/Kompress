...
class TarArchiveInputStream : ArchiveInputStream {
    ...
    private lateinit var inputStream: SyncInputStream

    /** Input streams for reading sparse entries  */
    private var sparseInputStreams: MutableList<SyncInputStream> = mutableListOf()
     ...
     */
    constructor(inputStream: SyncInputStream) : this(
        inputStream,
     ...
     * @since 1.19
     */
    constructor(inputStream: SyncInputStream, lenient: Boolean) : this(
        inputStream,
        TarConstants.DEFAULT_BLKSIZE,
        TarConstants.DEFAULT_RCDSIZE,
        null,
     ...
     */
    constructor(inputStream: SyncInputStream, blockSize: Int) : this(
        inputStream,
        blockSize,
        TarConstants.DEFAULT_RCDSIZE
    )
     ...
     */
    constructor(inputStream: SyncInputStream, blockSize: Int, recordSize: Int) : this(
        inputStream,
        blockSize,
        recordSize,
        null
     ...
     */
    constructor(
        inputStream: SyncInputStream, blockSize: Int, recordSize: Int,
        encoding: String?,
     ...
     */
    constructor(
        inputStream: SyncInputStream, blockSize: Int,
        encoding: String?,
     ...
     * @since 1.4
     */
    constructor(inputStream: SyncInputStream, encoding: String?) : this(
    ...
    private fun buildSparseInputStreams() {
        ...
        val zeroInputStream: SyncInputStream = TarArchiveSparseZeroInputStream() //NOSONAR
        ...
        for (sparseHeader in sparseHeaders) {
            ...
            if (zeroBlockSize > 0) {
                sparseInputStreams.add(BoundedInputStream(zeroInputStream, sparseHeader.offset - offset))
            }

            // only store the input streams with non-zero size
            if (sparseHeader.numbytes > 0) {
                sparseInputStreams.add(BoundedInputStream(inputStream, sparseHeader.numbytes))
            }
...
//    }
     ...
     */
    @Throws(IOException::class)
    fun getLongNameData(): ByteArray? {
        // read in the name
        val byteArrayBuilder = ByteArrayBuilder()
        val longName = MemorySyncStreamBase(byteArrayBuilder).open()
        var length = 0
     ...
     */
    @Throws(IOException::class)
    fun getNextTarEntry(): TarArchiveEntry? {
        if (isAtEOF()) {
            return null
        }
        if (currEntry != null) {
     ...
     */
    @Throws(IOException::class)
    private fun getRecord(): ByteArray? {
    ...
    private fun paxHeaders() {
        ...
        val headers: Map<String, String> = TarUtil.parsePaxHeaders(this, sparseHeaders, globalPaxHeaders, entrySize)
        ...
        if (currEntry!!.isPaxGNU1XSparse()) {
            sparseHeaders = TarUtil.parsePAX1XSparseHeaders(inputStream, recordSize).toMutableList()
            currEntry!!.setSparseHeaders(sparseHeaders)
        }
    ...
    private fun readGlobalPaxHeaders() {
        globalPaxHeaders = TarUtil.parsePaxHeaders(this, globalSparseHeaders, globalPaxHeaders, entrySize).toMutableMap()
        getNextEntry() // Get the actual file entry
        if (currEntry == null) {
    ...
    fun readRecord(): ByteArray? {
        val readNow: Int = inputStream.read(recordBuffer)//IOUtils.readFully(inputStream, recordBuffer)
        count(readNow)
     ...
     */
    @Throws(IOException::class)
    private fun readSparse(buf: ByteArray, offset: Int, numToRead: Int): Int {
...
//        val availableOfInputStream: Long = inputStream.available().toLong()
        ...
        val numToSkip: Long = min(count.toLong(), available)
        var skipped: Long
...
//            val available: Long = inputStream.available().toLong()
     ...
     */
    @Throws(IOException::class)
    private fun skipSparse(n: Long): Long {
        if (sparseInputStreams == null || sparseInputStreams.isEmpty()) {
...
//        val marked: Boolean = inputStream.markSupported()
        val markableStream = inputStream.markable()
//        if (marked) {
